...
/* ----------------------------------------------------------------------------
   ...
   CMS-friendly. Build-time responsive optimization is layered on in Phase 1.
   -------------------------------------------------------------------------- */

const imageRef = z.object({
  src: z.string(), // public path, e.g. /images/skeletals/<descriptive-name>.png
  alt: z.string(), // descriptive alt text (specimen + view + length) — hard rule §3
});
...
const taxa = defineCollection({
  ...
  schema: z.object({
    ...
    authority: z.string(),              // e.g. "Osborn, 1905"
    gallery: z.string(),                // section bucket / URL parent, e.g. "theropods"
    clade: z.array(z.string()),         // ORDERED cladistic path, broad → specific
    family: z.string().optional(),      // for the gallery card "family" line
    specimenId: z.string().optional(),  // single-specimen taxa, e.g. "USNM 555000"
    specimenName: z.string().optional(),// e.g. "The Nation's T. rex"
    alsoKnownAs: z.array(z.string()).default([]),
    lengthM: z.number().optional(),     // numeric — for sorting + length badges
    lengthLabel: z.string().optional(), // display override, e.g. "~12 m"
    ...
    drawingCredit: z.string().optional(), // e.g. "© Scott Hartman, 2023"
    drawingYear: z.number().optional(),
    reconstruction: imageRef,            // always present (the hero)
    rigorous: imageRef.optional(),       // PRESENCE ⇒ hasRigorous (derived, not stored)
